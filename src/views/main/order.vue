...
export default defineComponent({
  ...
  setup() {
    ...
    const handleQueryPassenger = () => {
      axios.get("http://localhost:8000/member/passenger/query-mine").then((response) => {
    ...
    const handleOk = () => {
      ...
      axios.post("http://localhost:8000/business/confirm-order/do", {
    ...
    const queryLineCount = () => {
      ...
      queryLineCountInterval = setInterval(function () {
        axios.get("http://localhost:8000/business/confirm-order/query-line-count/" + confirmOrderId.value).then((response) => {
          let data = response.data;
          if (data.success) {
    ...
    const loadImageCode = () => {
      ...
      imageCodeSrc.value = "http://localhost:8000/business/kaptcha/image-code/+ imageCodeToken.value";
    };
     ...
     */
    const onCancelOrder = () => {
      axios.get("http://localhost:8000/business/confirm-order/cancel/" + confirmOrderId.value).then((response) => {
        let data = response.data;
        if (data.success) {
          let result = data.content;
          if (result === 1) {
